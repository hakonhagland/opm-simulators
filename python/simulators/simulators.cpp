...
public:
    ...
    int step_cleanup() {
        auto result = mainfunc_->execute_last_step();
        hasRunInit_ = false;
        return result;
    }
    ...
    int step_init()
    {
        ...
        mainfunc_ = Opm::flowEbosBlackoilMain_init(argc, argv);
        int result = mainfunc_->execute_init_step(argc, argv);
    ...
    {
        if (!hasRunInit_) {
            int result = step_init();
            if (result == EXIT_FAILURE) {
                return EXIT_FAILURE;
            }
        }
        stepCounter_++;  // advance counter for each call
        return mainfunc_->execute_step();
        //return Opm::flowEbosBlackoilMain_step(argc, argv);
    }
...
PYBIND11_MODULE(simulators, m)
{
    py::class_<BlackOilSimulator>(m, "BlackOilSimulator")
        ...
        .def("step_cleanup", &BlackOilSimulator::step_cleanup)
